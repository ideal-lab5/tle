...
use etf_crypto_primitives::{encryption::tlock::*, ibe::fullident::*};
use rand_core::OsRng;
use w3f_bls::{EngineBLS, TinyBLS377};

/// encrypts a message for the identity and then performs decryption on
/// threshold sigs represents the worst case scenario where the threshold equals
...
fn tlock_dynamic_commitee_static_data(c: &mut Criterion) {
	...
	for size in [3, 5, 10, 20, 50, 100].iter() {
		let (round_secret, shares) = generate_secrets::<TinyBLS377, OsRng>(
			*size as u8,
			*size as u8,
			&mut OsRng,
		);
		let p_pub = <TinyBLS377 as EngineBLS>::PublicKeyGroup::generator() *
			round_secret;
