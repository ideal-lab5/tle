	...
	R: Rng + CryptoRng,
{
	...
	let message_ciphertext = S::encrypt(message, secret_key, &mut rng)
		.map_err(|_| Error::MessageEncryptionError)?; // not sure how to test this line...
	let mut message_bytes = Vec::new();
	message_ciphertext.serialize_compressed(&mut message_bytes).unwrap(); // TODO

	Ok(TLECiphertext {
		header,
		message_ciphertext: message_bytes,
	...
	S: StreamCipherProvider<32>,
{
	...
	let ct = S::Ciphertext::deserialize_compressed(
		&mut &ciphertext.message_ciphertext.clone()[..],
	)
	.unwrap(); // TODO
...
mod test {
	...
	use super::*;
	use crate::{
		curves::drand::TinyBLS381, stream_ciphers::AESGCMStreamCipherProvider,
	};
	use alloc::vec;
	use ark_ec::Group;
	...
	fn tlock_test<E: EngineBLS, R: Rng + Sized + CryptoRng>(
		...
		match tle::<E, AESGCMStreamCipherProvider, OsRng>(
			...
			Ok(mut ct) => {
				...
				if inject_bad_ct {
					let mut output = AESOutput::deserialize_compressed(
						&mut &ct.message_ciphertext[..],
					)
					.unwrap();
					output.ciphertext = vec![];
					let mut corrupted = Vec::new();
					output.serialize_compressed(&mut corrupted).unwrap();
					ct.message_ciphertext = corrupted;
				}
	...
	// 	let result = ct.tld::<AESGCMStreamCipherProvider>(sig).unwrap();
